  ...
  true ->receive
              {Main,Node,Cardinality} -> logger:log(warning,"=========updating request sent by main server for set ~p with cardinality ~p======",[Node,Cardinality]),
                                        % Time1 = os:system_time(),
                                         ...
                                         {Time,Result} = timer:tc(fun()-> lasp:read(node_util:atom_to_lasp_identifier(Node, state_gset), {cardinality, Cardinality}) end),
                                         %Time = os:system_time(),
                                         %logger:log(warning,"====printing time after read ~p",[Time]),
                                         %T = Time - Time1,
                                         %TotalTime = T/1000000,
                                         logger:log(warning,"==============Time for blocking read is ~p============",[Time /1000000]),
